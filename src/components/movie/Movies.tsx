...
export default function AdminMediaLibrary({ initialData }: MediaLibraryProps) {
  ...
  return (
    // ১. মেইন কন্টেইনারের ব্যাকগ্রাউন্ড ডাইনামিক করা হয়েছে
    <div className="lg:p-4 p-0 md:p-8 bg-zinc-50 dark:bg-[#020617] min-h-screen text-zinc-600 dark:text-zinc-300 font-sans selection:bg-emerald-500/30 transition-colors duration-300">
      <div className="max-w-7xl mx-auto space-y-10">

        {/* Header & Search */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="space-y-2">
            <h1 className="lg:text-3xl md:text-2xl text-xl font-black text-zinc-900 dark:text-white tracking-tighter flex items-center gap-3">
              <Film className="text-emerald-600 dark:text-emerald-500 lg:w-10 h-10" /> EXPLORE MEDIA
            </h1>
            <p className="text-zinc-500 dark:text-zinc-500 text-xs md:text-sm font-medium uppercase tracking-[0.1em]">Discover Premium Entertainment</p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="relative flex-1 lg:w-80 group">
              ...
              <input
                ...
                className="bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-white/30 rounded-2xl lg:py-4 py-3 pl-12 pr-4 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/40 w-full transition-all shadow-sm dark:shadow-none"
              />
            </div>
            <button
        ...
        {/* Filters Bar */}
        <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-white/[0.02] lg:p-3 rounded-[2.5rem] border border-zinc-200 dark:border-white/5 backdrop-blur-xl shadow-sm dark:shadow-none gap-4">
          ...
          <div className="flex lg:flex-wrap items-center gap-4">
            {/* Type Select */}
            <div className="flex items-center gap-3 bg-zinc-100 dark:bg-black/20 px-4 py-3 rounded-2xl border border-zinc-200 dark:border-white/5 group">
              <Clapperboard size={14} className="text-emerald-600 dark:text-emerald-500 group-hover:text-emerald-400 transition-colors" />
              <select
                value={searchParams.get('type') || ''}
                onChange={(e) => e.target.value === '' ? clearFilter('type') : handleFilterChange('type', e.target.value)}
                className="bg-transparent text-zinc-900 dark:text-white text-[10px] font-black uppercase tracking-widest focus:outline-none cursor-pointer"
              >
                <option value="" className="bg-white dark:bg-zinc-950">All Types</option>
                <option value="MOVIE" className="bg-white dark:bg-zinc-950">Movie</option>
                <option value="SERIES" className="bg-white dark:bg-zinc-950">Series</option>
                <option value="ANIMATION" className="bg-white dark:bg-zinc-950">Animation</option>
              </select>
            </div>

            {/* Sort By Select */}
            <div className="flex items-center gap-4 bg-zinc-100 dark:bg-black/20 px-6 py-3 rounded-2xl border border-zinc-200 dark:border-white/5">
              ...
              <select
                ...
                className="bg-transparent text-zinc-900 dark:text-white text-xs font-bold focus:outline-none cursor-pointer"
              >
                <option value="newest" className="bg-white dark:bg-zinc-950">Recently Added</option>
                <option value="priceHigh" className="bg-white dark:bg-zinc-950">Price: High to Low</option>
                <option value="priceLow" className="bg-white dark:bg-zinc-950">Price: Low to High</option>
                <option value="rating" className="bg-white dark:bg-zinc-950">Top Rated</option>
              </select>
            </div>
          </div>
        </div>
        ...
        {movies.length > 0 ? (
                ...
                <div className="relative aspect-[2/3] overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex flex-col gap-2">
                    {movie.contentType === 'PREMIUM' ? (
                      ...
                      <span className="bg-white/20 backdrop-blur-md text-white text-[10px] font-black px-3 py-1.5 rounded-xl border border-white/20">
                        FREE
                      </span>
                    )}
                  </div>
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {movie.genre?.slice(0, 2).map((g: string) => (
                      <span key={g} className="text-[9px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-tighter">
                        {g}
                      </span>
                    ))}
                  </div>
          ...
          <div className="py-40 text-center space-y-4">
            <div className="w-20 h-20 bg-zinc-100 dark:bg-zinc-900 rounded-full flex items-center justify-center mx-auto border border-zinc-200 dark:border-white/5">
              <Film className="text-zinc-400 dark:text-zinc-700" size={32} />
            </div>
            <p className="text-zinc-400 dark:text-zinc-600 font-black uppercase tracking-[0.3em] text-sm">No Media Found</p>
          </div>
        )}

        {/* Pagination */}
        <div className="pt-10 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-zinc-200 dark:border-white/5">
          <div className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">
            Showing <span className="text-zinc-900 dark:text-white">{movies.length}</span> — Page {meta.page} of {meta.totalPage}
          </div>
