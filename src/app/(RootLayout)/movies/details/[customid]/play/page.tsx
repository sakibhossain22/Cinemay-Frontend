...
import { Download, ListVideo, Lock, Calendar, Star, User, Play } from 'lucide-react';
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-1 lg:pl-4">
                        <div className="w-full bg-black shadow-2xl relative">
                            <div className="relative w-full aspect-video max-h-[80vh] mx-auto bg-zinc-100 dark:bg-zinc-900 overflow-hidden">
                                {hasPurchased ? (
                                    <iframe
                    ...
                    <div className="w-full lg:w-[400px] p-4 lg:pr-6 space-y-6">
                        ...
                        <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                            <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900/80 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <ListVideo size={16} className="text-emerald-500" />
                                    <h3 className="font-black uppercase tracking-widest text-[10px] text-zinc-500 dark:text-zinc-400">
                                    ...
                                    movieData?.episodeLinks.map((link: string, index: number) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full flex items-center gap-4 p-4 transition-all border-b border-zinc-200 dark:border-zinc-800/30 hover:bg-zinc-200 dark:hover:bg-white/5 ${isActive ? 'bg-emerald-500/10 border-l-4 border-l-emerald-500' : ''}`}
                                            >
                                                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 font-black text-xs ${isActive ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-500'}`}>
                                                    {index + 1}
                                                </div>
                                                <div className="text-left">
                                                    <p className={`text-[11px] font-black uppercase ${isActive ? 'text-emerald-500' : 'text-zinc-800 dark:text-zinc-300'}`}>Episode {index + 1}</p>
...
function SidebarInfoItem({ icon, label, value }: { icon: any, label: string, value: string }) {
    return (
        <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/60 p-4 rounded-xl flex items-center gap-4 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
            <div className="w-10 h-10 bg-zinc-200 dark:bg-zinc-800 rounded-lg flex items-center justify-center text-zinc-500">
                {icon}
            </div>
            <div>
                <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">{label}</p>
                <p className="text-xs font-bold text-zinc-800 dark:text-zinc-200 uppercase">{value}</p>
            </div>
        </div>
    );
}