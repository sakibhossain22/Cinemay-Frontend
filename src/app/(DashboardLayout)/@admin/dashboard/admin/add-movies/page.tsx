...
import {
    ...
    Globe, DollarSign, Film, User, Layers, List
} from "lucide-react";
import { toast } from "sonner";
import { useRouter } from "next/navigation";
import { addMovie, getTheMovieDB } from "@/actions/movieAction";
...
export default function AddMovies() {
    ...
    const [formData, setFormData] = useState({
        ...
        streamingLink: "",
        downloadLink: "",
        episodeLinks: "", // UI-তে স্ট্রিং হিসেবে থাকবে
        contentType: "PREMIUM", // FREE | PREMIUM
        buyPrice: 0,
        rentPrice: 0,
        rentDuration: 48,
    });

    // Access Type 'FREE' হলে প্রাইস ০ করে দেওয়ার জন্য ইফেক্ট
    useEffect(() => {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setFormData((prev) => ({
                    ...
                    contentType: movie.contentType || "PREMIUM",
                    buyPrice: movie.buyPrice || 0,
                    rentPrice: movie.rentPrice || 0,
                    ratingAverage: movie.ratingAverage || 0
                }));

                toast.success("Movie details fetched from Server!");
            } else {
                toast.error("Movie not found in TMDB database");
            }
        } catch (error) {
            console.error(error);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // স্ট্রিং ইনপুটগুলোকে অ্যারেতে রূপান্তর
        const finalData = {
            ...formData,
            genre: formData.genre.split(",").map(g => g.trim()).filter(g => g !== ""),
            cast: formData.cast.split(",").map(c => c.trim()).filter(c => c !== ""),
            episodeLinks: formData.episodeLinks.split(",").map(l => l.trim()).filter(l => l !== ""),
        };

        try {
            // console.log("Submitting to your DB:", finalData);
            // // এখানে আপনার অ্যাড মুভি সার্ভার অ্যাকশন কল করুন
            const response = await addMovie(finalData);

            if (!response.success) {
                toast.error(response.error || "Failed to save movie");
                return;
            }
    ...
    const inputStyle = "w-full bg-zinc-900/50 border border-white/5 rounded-2xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 transition-all placeholder:text-zinc-700 text-sm disabled:opacity-30 disabled:cursor-not-allowed";
        ...
        <div className="min-h-screen bg-black text-white p-6 md:p-12">
            <div className="max-w-6xl mx-auto space-y-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div>
                        <h1 className="text-5xl font-black tracking-tighter italic">ADD <span className="text-emerald-500">MEDIA</span></h1>
                        <p className="text-zinc-500 font-bold mt-2 uppercase text-xs tracking-widest">Inventory Management System</p>
                    </div>
                    <button
                        form="movie-form"
                        disabled={loading}
                    ...
                    <div className="flex-1 w-full space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="Paste TMDB ID here (e.g. 76600)"
                    ...
                    <div className="md:col-span-8 space-y-8">
                        ...
                        <div className="bg-zinc-900/20 border border-white/5 p-8 rounded-[2.5rem] space-y-6">
                            ...
                            <div className="space-y-4">
                                <label className={labelStyle}>Poster Image URL</label>
                                <input className={inputStyle} value={formData.posterUrl} onChange={(e) => setFormData({ ...formData, posterUrl: e.target.value })} />

                                <div className="grid grid-cols-2 gap-6 pt-2">
                                    <div>
                                        <label className={labelStyle}>Streaming (Embed)</label>
                                        <input className={inputStyle} placeholder="Trailer or Movie Link" value={formData.streamingLink} onChange={(e) => setFormData({ ...formData, streamingLink: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className={labelStyle}>Direct Download Link</label>
                                ...
                                <div className="pt-2">
                                    ...
                                    <textarea
                                        rows={3}
                                        className={`${inputStyle} resize-none font-mono text-[12px]`}
                        ...
                        <div className="bg-zinc-900/20 border border-white/5 p-8 rounded-[2.5rem] space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea rows={4} className={`${inputStyle} resize-none`} placeholder="Names separated by comma" value={formData.cast} onChange={(e) => setFormData({ ...formData, cast: e.target.value })} />
                                </div>
                            </div>
                        </div>

                        <div className={`bg-zinc-900/20 border p-8 rounded-[2.5rem] space-y-6 transition-all ${formData.contentType === "FREE" ? "border-white/5 opacity-60" : "border-emerald-500/20"}`}>
                            <h3 className="flex items-center gap-2 font-bold text-emerald-500 uppercase text-xs tracking-widest"><DollarSign size={16} /> Pricing & Access</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className={labelStyle}>Access Tier</label>
                                    <select className={inputStyle} value={formData.contentType} onChange={(e) => setFormData({ ...formData, contentType: e.target.value })}>
                                        ...
                                        <option value="FREE">FREE (No Price)</option>
                                    </select>
                                </div>
                                <div>
